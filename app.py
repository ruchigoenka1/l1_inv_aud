...
def run_full_audit(df, lt, u_val, h_pct, o_cost):
    """Historical Performance Audit Logic."""
...
def run_prophet_dna(df):
    """AI DNA Extraction & Smoothed Stratification."""
    ...
    # 14-day smoothing to create 'Coherent Seasonal Blocks'
...
fixed_wh_cost = st.sidebar.number_input("Fixed Storage Cost per Unit/Year ($)", value=10.0, 
                                       help="Total annual Rent + Labor + Equipment divided by Total Capacity")
...
if uploaded_file:
    ...
    with tab2:
        ...
        if 'dna_df' in st.session_state:
            ...
            st.plotly_chart(px.scatter(df, x='Date', y='Demand', color='Zone', color_discrete_map={"Peak Season": "#F56565", "Normal Season": "#63B3ED", "Low Season": "#4FD1C5"}, title="Demand Zones & Trend", template="plotly_dark"), use_container_width=True)
            
            c1, c2 = st.columns(2)
            with c1: risk_window = st.slider("Lead Time Window", 1, 30, int(lt_manual))
            with c2: target_sl = st.slider("Target Service Level (%)", 80, 99, 95) / 100
            
            df['Rolling_Demand'] = df['Demand'].rolling(window=risk_window).sum()
            st.subheader("📊 Probability Distribution (Lead-Time Demand)")
            st.plotly_chart(px.histogram(df.dropna(subset=['Rolling_Demand']), x="Rolling_Demand", color="Zone", barmode='overlay', opacity=0.6, template="plotly_dark"), use_container_width=True)
    ...
    with tab3:
        if 'zone_stats' not in st.session_state:
            ...
            if st.button("▶️ Run Stress Test"):
                ...
                # Financial Math (Fixed + Var)
                annual_h_unit = (u_val * (h_pct/100)) + fixed_wh_cost
                daily_h_rate = annual_h_unit / 365
                
                stocks, shortages, sim_demands, in_lt_sim, orders_placed = [], [], [], [], []
                ...
                for d in range(sim_days):
                    ...
                    shrt = int(daily_d - sales) # Syncing units/revenue
                    ...
                    in_lt_sim.append(len(p_orders) > 0); orders_placed.append(placed)
                    stock = close_s
                ...
                # --- ADVANCED KPIs ---
                st.subheader("Operational & Financial Results")
                k_c1, k_c2, k_c3, k_c4 = st.columns(4)
                k_c1.metric("Stockout Days", int((sdf['Shortage'] > 0).sum()))
                fill_rate = (1 - (sdf['Shortage'].sum() / sdf['Demand'].sum())) * 100 if sdf['Demand'].sum() > 0 else 100
                k_c2.metric("Global Fill Rate", f"{fill_rate:.1f}%")
                
                lt_sim_fr = (1 - (sdf[sdf['InLT']]['Shortage'].sum() / sdf[sdf['InLT']]['Demand'].sum())) * 100 if not sdf[sdf['InLT']].empty else 100
                k_c3.metric("LT Fill Rate", f"{lt_sim_fr:.1f}%")
                
                lost_rev = int(sdf['Shortage'].sum()) * u_val
                k_c4.metric("Lost Sales Revenue", f"${lost_rev:,.0f}", delta=f"{int(sdf['Shortage'].sum())} Units", delta_color="inverse")

                st.subheader("Cost Analysis & Suggested EOQ")
                f1, f2, f3, f4 = st.columns(4)
                tot_h = sdf['Stock'].sum() * daily_h_rate
                tot_o = (sdf['Placed'] > 0).sum() * o_cost
                f1.metric("Total Holding Cost", f"${tot_h:,.0f}")
                f2.metric("Total Ordering Cost", f"${tot_o:,.0f}")
                ...
                st.plotly_chart(px.line(sdf, x="Day", y=sdf['Stock']*u_val, color_discrete_sequence=['#FFA500'], template="plotly_dark"), use_container_width=True)
